...
`data/db.json`을 초기화하고 샘플 프로젝트(**CES 2027 Press Conference**)를 생성합니다. seed를 건너뛰어도 서버 최초 요청 시 샘플 프로젝트가 자동 생성됩니다.
...
2. 홈에서 **CES 2027 Press Conference** 프로젝트 클릭
3. **타임테이블 · 구조** 탭 — 9개 프로그램과 13개 자원 확인, 각 항목 수정 가능
4. **사전 위험 진단** 탭 — "위험 진단 실행"으로 결정론적 진단 실행 (시드된 6개 위험 + 자동 진단 결과 병합)
5. **시뮬레이션** 탭 — 난도·구간·빈도·리스크 범주를 고르고 **GO**
6. 상황 카드에서 선택지를 고르거나 직접 대응안 입력 → 점수 변화와 근거 확인 → 일부 선택은 연쇄 상황 유발
7. 모든 상황 처리 후(또는 "시뮬레이션 종료" 버튼) 결과 리포트 확인, Markdown 내보내기
