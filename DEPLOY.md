...
이 앱은 데이터를 서버의 JSON 파일(`data/db.json`)에 저장합니다. **무료 플랜은 영구 디스크(disk)를 지원하지 않아** `render.yaml`에서 해당 설정을 뺐습니다 — 즉, 서버가 재시작되거나 재배포될 때마다 데이터가 시드 상태로 초기화됩니다. 발표 도중 만든 프로젝트/시뮬레이션 결과도 서버 재시작 시 사라질 수 있다는 뜻이니, 데모는 한 세션 안에서 끝내는 걸 권장합니다.

데이터를 계속 유지하고 싶다면 Render 대시보드에서 플랜을 **Starter** 이상으로 올리고 `render.yaml`에 아래 블록을 다시 추가하세요 (파일 안에 예시로 남겨뒀습니다):
```yaml
disk:
  name: showrunner-data
  mountPath: /opt/render/project/src/data
  sizeGB: 1
```
실제 운영에는 이보다 Prisma + PostgreSQL 전환을 더 권장합니다 (README의 "데이터 레이어에 관한 안내" 참고).
